...
# The assignment:
# Below you find the simplified HTML of Richard Branson's My top 10 quotes on 
# living life better.
...
def extract_quotes(html: str = HTML) -> dict:
    """Extract quotes from the HTML content and return a dictionary of author-quote pairs"""
    lines = html.splitlines()
    quotes_dict = {}

    for line in lines:
        result = re.search(r"^\s*<p>[0-9]+\. \"([\w ,’.…–;]*)\" - ([\w \.]*)</p>$", line)
        if result:
            # Extract the quote and author using capturing groups in the regex
            quote = result[1]
            author = result[2]
