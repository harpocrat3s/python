...
# This is another simple Python exercise I did while learning the language
# on the great codechalleng.es platform, where I did a lot of so called 
# "PyBites". As per the assignment: 
# In this Bite you complete the function get_users_for_shell that takes 
# a /etc/passwd multiline string and a shell to filter on (default bash). 
...
def get_users_for_shell(passwd_output: str = PASSWD_OUTPUT,
    ...
    """
    usr_list = []
    passwd_list = passwd_output.splitlines()

    for line in passwd_list:
        line_tmp = line.split(':')
        if line_tmp[-1].endswith('/' + grep_shell):
            usr_list.append(line_tmp[0])
